...
def getImageInfo(imag, prodcut):
    path1 = r"TestingFolder"
    os.chdir(path1)
    with open ('pcaModel_' + prodcut + '.pkl', 'rb') as fp:
            pca_model = pickle.load(fp)
    with open ('kmeansModel_' + prodcut + '.pkl', 'rb') as fp:
            kmeans_model = pickle.load(fp)
    with open ('kmeansDictionary_' + prodcut + '.pkl', 'rb') as fp:
    ...
    if(prodcutName == 'No Match'):
        return 'No Match'
    else:
        return (ProductsDict[prodcutName])
